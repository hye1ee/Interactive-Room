...
export default function Model(props) {
  const group = useRef()
  const { nodes, materials } = useGLTF('/room_final.gltf')

  const [planeRef] = usePlane(()=>({ type :'Static', rotation : [-Math.PI / 2, 0, 0],  position : [0,10,0]}));
  
  const [target] = useState(() => new THREE.Object3D());

  const [trashScale, setTrashScale] = useState(1);

  const [shoe1Ref] = useBox(() => ({ mass: 1, position : [0,30,0], linearDamping: 0.95, angularDamping: 0.95 }));
  ...
  return (
    <group ref={group} {...props} dispose={null}>
      ...
      <group  ref={shoe1Ref}  dispose={null}>
          <mesh  geometry={nodes.shoe_bottom2.geometry} material={nodes.shoe_bottom2.material}>
            <boxGeometry />
          </mesh>
          <mesh geometry={nodes.shoe_top2.geometry} material={nodes.shoe_top2.material} position={[23.85-22.16, 7.75-6.1, 22.13-21.51]} />
      </group>
      ...
      <mesh geometry={nodes.shoe_top1_1.geometry} material={nodes.shoe_top1_1.material} position={[26.6, 7.75, 10.8]} />
...
useGLTF.preload('/room_final.gltf')
